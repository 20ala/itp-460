<?php
	...
	$sql = "SELECT flights.price AS 'price', 
				flights.start_date AS 'start_date', 
				airlines.airline_name AS 'airline',
                team_alias_1.city_name AS 'from', 
				team_alias_2.city_name AS 'to'  
			FROM flights
			LEFT JOIN airlines
				ON flights.airline_id = airlines.id
            LEFT JOIN cities AS team_alias_1
				ON flights.from_city_id = team_alias_1.id
			LEFT JOIN cities AS team_alias_2
				ON flights.to_city_id = team_alias_2.id
			WHERE 1 = 1";

	if (isset($_GET['airline_id']) && trim($_GET['airline_id']) != '') {
		$airline_id = $_GET['airline_id'];
		$sql = $sql . " AND flights.airline_id = $airline_id";
	}

    if (isset($_GET['from_city_id']) && trim($_GET['from_city_id']) != '') {
		$from_city_id = $_GET['from_city_id'];
		$sql = $sql . " AND team_alias_1.id = $from_city_id";
	}
    if (isset($_GET['to_city_id']) && trim($_GET['to_city_id']) != '') {
		$from_city_id = $_GET['to_city_id'];
		$sql = $sql . " AND team_alias_2.id = $to_city_id";
	}

    if (isset($_GET['from']) && trim($_GET['from']) != '') {
		$from = $_GET['from'];
		$sql = $sql . " AND flights.start_date = $from";
	}

	if (isset($_GET['price']) && trim($_GET['price']) != '') {
		$price = $_GET['price'];
		$sql = $sql . " AND flights.price <= $price";
	}
    ...
    <body>
		...
		<div class="row mb-4">
			<div class="col-12 mt-4">
				<a href="flight_form.php" role="button" class="btn btn-primary">Back to Form</a>
			</div> <!-- .col -->
		</div> <!-- .row -->
		<div class="row">
			<div class="col-12">
