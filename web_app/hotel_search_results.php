<?php
	...
	$sql = "SELECT hotels.price AS 'price', 
				hotels.start_date AS 'start_date', 
				hotel_companies.company_name AS 'hotel', 
				cities.city_name AS 'city_name' 
			FROM hotels
			LEFT JOIN hotel_companies
				ON hotels.company_id = hotel_companies.id
			LEFT JOIN cities
				ON hotels.city_id = cities.id
			WHERE 1 = 1";

	if (isset($_GET['chain_id']) && trim($_GET['chain_id']) != '') {
		$chain_id = $_GET['chain_id'];
		$sql = $sql . " AND hotels.company_id = $chain_id";
	}

    if (isset($_GET['city_id']) && trim($_GET['city_id']) != '') {
		$city_id = $_GET['city_id'];
		$sql = $sql . " AND hotels.city_id = $city_id";
	}

    if (isset($_GET['from']) && trim($_GET['from']) != '') {
		$from = $_GET['from'];
		$sql = $sql . " AND hotels.start_date = $from";
	}

	if (isset($_GET['price']) && trim($_GET['price']) != '') {
		$price = $_GET['price'];
		$sql = $sql . " AND hotels.price <= $price";
	}
    ...
    <body>
		...
		<div class="row mb-4">
			<div class="col-12 mt-4">
				<a href="hotel_form.php" role="button" class="btn btn-primary">Back to Form</a>
			</div> <!-- .col -->
		</div> <!-- .row -->
		<div class="row">
			<div class="col-12">
